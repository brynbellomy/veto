...
// TestFetchGuttedGobMustNotSilentlyShrink: damaged gob bytes must fail the
// content-hash check and fall through to re-parse — never silently decode
// into a smaller report set.
func TestFetchGuttedGobMustNotSilentlyShrink(t *testing.T) {
	...
	warmReports, err := warm.Fetch(context.Background(), intel.EcosystemCrates)
	require.NoError(t, err)
	...
	reports, err := cold.Fetch(context.Background(), intel.EcosystemCrates)
...
func TestFetchGuttedTarballMustNotSilentlyShrink(t *testing.T) {
	...
	_, err = warm.Fetch(context.Background(), intel.EcosystemCrates)
	require.NoError(t, err)

	require.NoError(t, os.WriteFile(
		filepath.Join(cacheDir, "osv.tar.gz"), []byte("gutted tarball"), 0o600))
	entries, err := os.ReadDir(cacheDir)
	require.NoError(t, err)
	for _, e := range entries {
		if filepath.Ext(e.Name()) == ".gob" {
