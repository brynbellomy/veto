...
func TestProjectPreflight(t *testing.T) {
	...
	cases := []struct {
		...
		args    []string
		wantOK  bool
		wantRef bool // true → expect package.json + lockfile refs
	}{
		...
		{name: "ci is not a preflight verb", args: []string{"ci"}, wantOK: false},
		{name: "flag-with-value before verb still finds run", args: []string{"--prefix", "/tmp", "run", "dev"}, wantOK: true, wantRef: true},
