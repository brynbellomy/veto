...
func TestProjectPreflight(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "x is not a preflight verb (fetch verb)", args: []string{"x", "some-tool"}, wantOK: false},
		{name: "create is not a preflight verb (fetch verb)", args: []string{"create", "app"}, wantOK: false},
